...
exports.getAllHistoryApiControl = asyncHandler(async (req, res) => {
  const history = await findHandler('UserGameHistory', [], 'all')
  return res.json(history)
})

exports.getOneHistoryApiControl = asyncHandler(async (req, res) => {
  const paramsUuid = req.params.uuid
  const history = await findHandler('UserGameHistory', [], 'one', paramsUuid)
  res.json(history)
})
...
exports.createHistoryApiControl = asyncHandler(async (req, res) => {
  ...
  const user = await findHandler('UserGame', [], 'one', user_uuid)
  console.log(user.id)
  if (!user || user === null) {
